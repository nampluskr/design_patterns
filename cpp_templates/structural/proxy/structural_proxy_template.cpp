...
// ProtectionProxy 클래스 (보호 프록시)
struct ProtectionProxy : Subject {
    ProtectionProxy(Subject* _subject, bool canAccess) : _subject(_subject), canAccess(canAccess) {}
    ~ProtectionProxy(){ delete _subject; }
...
int main()
{
    ...
    Subject* protectionProxy1 = new ProtectionProxy(realSubjectInstance, true); // 접근 권한 있음
    Subject* protectionProxy2 = new ProtectionProxy(new RealSubject(), false);  // 접근 권한 없음
    cout << endl;
